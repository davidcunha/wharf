...
SQliteAdapter.prototype.find = function(attrs, projectionAttrs) {
  ...
  if(validateAttrs.apply(this, [attrs, projectionAttrs])) {
    var selectionAttrs = selection(attrs);
    projectionAttrs = projection(projectionAttrs);
    return all('SELECT ' + projectionAttrs + ' FROM ' + this.tableName + ' ' + selectionAttrs).then(function(container) {
      return container;
    }).catch(function(err) {
      console.log(err.stack);
    });
...
SQliteAdapter.prototype.findAll = function(projectionAttrs) {
  projectionAttrs = projectionAttrs || [];
  if(validateAttrs.apply(this, [null, projectionAttrs])) {
    projectionAttrs = projection(projectionAttrs);
    return all('SELECT ' + projectionAttrs + ' FROM ' + this.tableName).then(function(containers) {
      return containers;
    }).catch(function(err) {
      console.log(err.stack);
    });
