...
describe('StatsWorker', function() {
  describe('.constructor', function() {
    ...
    it('returns error if process is missing', function() {
      expect(function() { new StatsWorker({id: 1, entityName: 'entityName'}) }).to.throw(Error, '\'process\' is undefined');
    });

    it('returns a valid worker', function() {
      expect(new StatsWorker({id: 1, entityName: 'entityName', process: {}})).to.be.ok;
    });
  });
});
